...
const accesorioIconMap = {
    ...
    Other: { icon: faComputer, bgColor: "cyan-500" },
};
...
const Accesorio: React.FC<AccesorioProps> = ({ accesorio, detalle, ticket, cantidad, id }) => {
    const { icon, bgColor } = accesorioIconMap[accesorio as keyof typeof accesorioIconMap] || { icon: faComputer, bgColor: "cyan-500" };
    ...
    return (
        <motion.div 
        className={`flex items-center bg-${bgColor} shadow-lg rounded-lg p-4 mb-4 cursor-pointer`} onClick={toggleDetail} onDoubleClick={handleDoubleClick}
        ...
        >
            ...
            <motion.div className="flex-1 transition-all duration-300 ease-in-out transform hover:scale-105"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            >
